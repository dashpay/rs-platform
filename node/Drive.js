...
class Drive {
  ...
  */
  async dequeueWithdrawals(useTransaction = false) {
    const withdrawalBuffers = await driveDequeueWithdrawalsAsync.call(
      this.drive,
      useTransaction,
    );

    return withdrawalBuffers.map((buffer) => cbor.decode(buffer));
  }
