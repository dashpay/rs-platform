...
pub struct IdentityUpdateTransitionBasic<T> {
    protocol_version_validator: Arc<ProtocolVersionValidator>,
    json_schema_validator: JsonSchemaValidator,
    public_keys_validator: Arc<T>,
}

impl<T: TPublicKeysValidator> IdentityUpdateTransitionBasic<T> {
    pub fn new(
        protocol_version_validator: Arc<ProtocolVersionValidator>,
        public_keys_validator: Arc<T>,
    ...
    pub fn validate(
        ...
        let raw_public_keys = raw_state_transition
            .get(property_names::ADD_PUBLIC_KEYS)
            .ok_or_else(|| {
                NonConsensusError::SerdeJsonError(format!(
                    "the {} doesn't exist in {:#?}",
