...
pub fn validate_public_keys(
    ...
    if !duplicated_ids.is_empty() {
        validation_result.add_error(DuplicatedIdentityPublicKeyIdError::new(duplicated_ids));
    }

    // Check that there's no duplicated keys
    let duplicated_key_ids = duplicated_keys(&public_keys);
    if !duplicated_key_ids.is_empty() {
        validation_result.add_error(DuplicatedIdentityPublicKeyError::new(duplicated_key_ids));
    }
