...
#[cfg(test)]
mod tests {
    use crate::common::{json_document_to_cbor, setup_contract, setup_contract_from_hex, cbor_from_hex, value_to_cbor};
    ...
    fn test_unknown_data_bug() {
        ...
        let (results, _) = drive
            .query_documents_from_contract(&contract, contract.document_types.get("niceDocument").unwrap(), query_cbor.as_slice(), None)
        ...
        let (results, _) = drive
            .query_documents_from_contract(&contract, contract.document_types.get("niceDocument").unwrap(), query_cbor.as_slice(), Some(&db_transaction))
            .expect("expected to execute query");
