...
mod tests {
    ...
    fn test_create_and_update_contract_without_apply() {
        ...
        let (storage_cost, processing_cost) = drive
            .apply_contract_cbor(initial_contract_cbor.clone(), None, 0f64, true, None)
            .expect("expected to apply contract successfully");

        // Create contract without apply

        let (no_apply_storage_cost, no_apply_processing_cost) = drive
            .apply_contract_cbor(initial_contract_cbor, None, 0f64, false, None)
            .expect("expected to apply contract successfully");

        assert!(storage_cost <= no_apply_storage_cost);
        assert!(processing_cost <= no_apply_processing_cost);
        ...
        let (actual_storage_cost, actual_processing_cost) = drive
            .apply_contract_cbor(updated_contract_cbor.clone(), None, 0f64, true, None)
            .expect("should update initial contract");

        // Update contract without apply

        let (predicted_storage_cost, predicted_processing_cost) = drive
            .apply_contract_cbor(updated_contract_cbor, None, 0f64, false, None)
            .expect("should update initial contract");
